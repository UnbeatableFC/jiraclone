...
const app = new Hono()
  ...
  .post(
    ...
    async (c) => {
      ...
      const highestPosistionTask = await databases.listDocuments(
        DATABASE_ID,
        TASKS_ID,
        [
      ...
      const newPosition =
        highestPosistionTask.documents.length > 0
          ? highestPosistionTask.documents[0].postion + 1000
          : 1000;
  ...
  .post(
    ...
    async (c) => {
      ...
      const workspaceId: string = String(
        workspaceIds.values().next().value
      );
